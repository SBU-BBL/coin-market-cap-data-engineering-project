...
def get_top_posts_from_api(url, headers, symbol=None, last_score=None):
    params = {
...
if __name__ == '__main__':
    ...
    while not user_symbol:
        user_symbol = input("Enter the cryptocurrency symbol you want to get top posts for (e.g., BTC, ETH): ").strip()
    ...
    url = config['API']['CONTENT']['POST_TOP']  # Adjust as per your config
    date = config['DATE']
    ...
    while True:
        batch_data, new_last_score = get_top_posts_from_api(url=url, headers=headers, symbol=user_symbol, last_score=last_score)
        if not batch_data:
            break
